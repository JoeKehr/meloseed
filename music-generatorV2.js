...
// Define an instrument (optional):
    ...
    for (let i = 0; i < chordsList.length * 12; ++i) {
        ...
        for (let j = 0, duration = 1; j < 1; j += (1 / duration)) {
            ...
            let pitch = chord[getRandomFromRange(0, chord.length - 1)] + "" + getRandomFromRange(3, 4);
            let note = {
                pitch: pitch,
                duration: duration,
                tension: false
            ...
            if (i > 0 && j > 0) {
                let tension = addTension(tab3[tab3.length - 1], note, j + (1 / duration), gamme, chord, getRandomFromRange(0, 4));
                for (let k = 0; k < tension.length; k++) {
                    tab3.push(tension[k]);
                    j += (1 / tension[k].duration);
                }
        ...
        for (let j = 0, duration = 1; j < 1; j += (1 / duration)) {
            ...
            let pitch = chord[getRandomFromRange(0, chord.length - 1)] + "" + getRandomFromRange(2, 3);
            let note = {
                pitch: pitch,
                duration: duration,
                tension: false
            ...
            if (i > 0 && j > 0) {
                let tension = addTension(tab4[tab4.length - 1], note, j + (1 / duration), gamme, chord, getRandomFromRange(0, 4));
                for (let k = 0; k < tension.length; k++) {
                    tab4.push(tension[k]);
                    j += (1 / tension[k].duration);
                }
...
//console.log(tab);
    ...
    var writer = new MidiWriter.Writer([track, track2 /*, track3, track4*/]);
    return {
...
module.exports.generateV2 = generateV2;